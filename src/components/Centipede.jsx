var Input = {
  ...
  function setupLizard(size, legs, tail) {
    ...
    setInterval(function() {
      ...
      // Definimos la zona segura del botón (zona central de la pantalla)
      const safeZoneWidth = window.innerWidth * 0.12; // 30% del ancho de la pantalla
      const safeZoneHeight = window.innerHeight * 0.1; // 30% del alto de la pantalla
      const safeZoneLeft = (window.innerWidth - safeZoneWidth) / 2;
      const safeZoneRight = safeZoneLeft + safeZoneWidth;
      const safeZoneTop = (window.innerHeight - safeZoneHeight) / 2;
      const safeZoneBottom = safeZoneTop + safeZoneHeight;

      // Coordenadas de la esquina superior izquierda (con un pequeño margen)
      const cornerX = window.innerWidth * 0.05; // 10% del ancho
      const cornerY = window.innerHeight * 0.1; // 10% del alto

      // Si el cursor está en la zona segura, el lagarto se mueve a la esquina
      if (Input.mouse.x > safeZoneLeft && Input.mouse.x < safeZoneRight && 
          Input.mouse.y > safeZoneTop && Input.mouse.y < safeZoneBottom) {
        critter.follow(cornerX, cornerY);
      } else {
        // Comportamiento normal fuera de la zona segura
        critter.follow(Input.mouse.x, Input.mouse.y);
      }
    }, 33);
  }
